...
#
# Phase 1 of docs/prompter-search-improvement-plan.md. No network, no model, no
...
if ! xcodebuild -project Greenroom.xcodeproj -scheme PrompterBench \
...
BIN=$(xcodebuild -project Greenroom.xcodeproj -scheme PrompterBench \
        -configuration Debug -showBuildSettings 2>/dev/null \
      | awk -F' = ' '/ BUILT_PRODUCTS_DIR/ {print $2; exit}')

exec "$BIN/PrompterBench" "$(pwd)/Bench/fixtures/detector.json" "$@"
